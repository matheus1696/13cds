<x-app-layout>
    <x-slot name="header">
        @if (isset($exercise))
            <x-header.title title="{{ __('Cadastro de Atividade Prática') }}" />
        @else
            <x-header.title title="{{ __('Alteração de Atividade Prática') }}" />
        @endif
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-6">
                ...
                <form method="POST" action="{{ isset($exercise) ? route('admin_exercises.update', $exercise->id) : route('admin_exercises.store') }}" class="space-y-6 mb-8">
                    @isset($exercise) @method('PUT') @endisset
                    @csrf
                    <!-- Title -->
                    <div class="mt-4">
                        <x-form.input-label for="title" :value="__('Title')" />
                        <x-form.input id="title" type="text" name="title" :value="old('title', $exercise->title ?? '')" required autofocus placeholder="Título da Atividade"/>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mt-4">
                        ...
                        <div>
                            ...
                            <x-form.input id="workload" type="number" name="workload" :value="old('workload', $exercise->workload ?? '')" min="1" placeholder="Carga Horária" required />
                        ...
                        <div>
                            ...
                            <x-form.input type="date" id="start_date" name="start_date" :value="old('start_date', $exercise->start_date ?? '')" required />
                        ...
                        <div>
                            ...
                            <x-form.input type="date" id="end_date" name="end_date" :value="old('end_date', $exercise->end_date ?? '')" required />
