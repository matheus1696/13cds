<x-app-layout>
    <x-slot name="header">
        <x-header.title title="{{ __('Avaliação') }} {{ __('Atividade Prática') }} - {{ __('Tema') }}: {{ $exerciseAnswer->Exercise->title ?? '' }}" />
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <section class="py-6">                    
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-justify">{{ $exerciseAnswer->description }}</p>
                        </div>

                        <div>
                            <h2 class="font-medium">Participantes:</h2>
                            @foreach ($exerciseAnswerUsers as $exerciseAnswerUser)
                                <p class="text-gray-600 text-justify">{{ $exerciseAnswerUser->User->name }}</p>
                            @endforeach
                        </div>
                    </div>
                </section>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('tutor_evaluation_exercises.update', $exerciseAnswer->id) }}" method="POST" class="space-y-6">
                    @csrf @method('PUT')
