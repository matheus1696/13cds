<x-app-layout>    
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <x-header.title title="{{ __('Comissão Organizadora') }}" />
            <x-button.link-primary href="{{ route('commissions.create') }}">
                ...
                <x-slot name="tbody">
                    @forelse ($commissions as $commission)
                        <tr class="hover:bg-gray-100">
                            ...
                            <!-- Whatsapp -->
                            <x-table.td class="py-3.5 text-center text-sm text-gray-700">
                                <p class=" rounded-full py-1 text-xs {{ $commission->is_whatsapp ? 'bg-green-700 text-green-100' : 'bg-red-700 text-red-100' }}">{{ $commission->is_whatsapp ? 'Sim' : 'Não' }}</p>
                            </x-table.td>

                            <!-- Ações -->
                            <x-table.td class="py-3.5 text-center">
                                <a href="{{ route('commissions.edit', $commission->id) }}" class="text-green-700 hover:text-green-60 px-2.5 py-1.5 rounded-full text-sm">
